...
public class Controller : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		float runMult = Input.GetKey(KeyCode.LeftShift) ? 10 : 1;

		if (Input.GetKey(forwardKey))
			move += Vector3.forward * moveSpeed * runMult;
		if (Input.GetKey(backKey))
			move += Vector3.back * moveSpeed * runMult;
		if (Input.GetKey(leftKey))
			move += Vector3.left * moveSpeed * runMult;
		if (Input.GetKey(rightKey))
			move += Vector3.right * moveSpeed * runMult;
